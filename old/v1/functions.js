...
function showFilter( field )
{
	if( field.value )
	{ 
		$('showtag').innerHTML = 'about ' + field.value + ' ';
	}
	else
	{
		$('showtag').innerHTML = '';
...
function flickrRequest( tag )
{
	...
	+ '&per_page='       + perPage
	+ '&page='           + page;
	//$('debug').innerHTML += '<br /><a href="' + apiURI + '?' + pars + '" target="_blank">XEMELE de resposta ' + groupId + '</a> ';//debug
...
function groupsController()
{
	...
	// if the groups are over, we have some photos but less than the user requested, and the page is less than X
	/*else if( !groupIds.length && photos.length >= perPage && photos.length <= numPhotos && page < 2 )
	{
		perPage *= 2; // apela e pede logo um milhão de fotos
		page++; //next page of results
		setGroups(); // reset the groups array
		flickrRequest( tag ); // magic (pó de prilimpimpim)
	}*/
	...
	/*
	if( photos.length >= numPhotos || !groupIds.length )
	{
		checkResult();
	}
	// if not reached the number requested and the photos are feel, re-search with other group
...
function checkResult()
{
	...
	else
	{
		...
		$('result').innerHTML = "<p>I'm just a <strong>nightly built software</strong> and I <strong>can't found</strong> any items macthing your criteria.</p><p>Help me being less specific.</p>"
	}
}
